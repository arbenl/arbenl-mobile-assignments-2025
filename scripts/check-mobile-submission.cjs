...
function parseSubmission(issue) {
  ...
  const week = issue.title.startsWith('[MOBILE J02]') ? 2 : Number(field(body, 'Java').match(/^Java ([12])(?:\s|$)/)?.[1]);
  if (![1,2].includes(week)) throw new Error('Zgjidh Java 1 ose Java 2 në formular.');
...
async function checkFiles(submission, github) {
...
function report({checks=[],ref,error}={}) {
...
async function run({github,context}) {
  ...
  let result;
  try {const submission=parseSubmission(issue);if(!submission)return;result=await checkFiles(submission,github);}
  catch(error) {
  ...
  // Only replace our bot's own report, never student comments.
